...
- 登录与账号权限
  - 支持账号密码登录。
  - 用户表为 `cw_ry`。
  - 登录态过期时间由 `app_config.json` 的 `login.expireHours` 控制，单位为小时。
  - 支持管理员、车队长、销售、财务四类权限。
  - 只有管理员可进入账号管理和推送配置。
...
| 成本明细（财务） | `cw_cbmxb` |
| 费用汇总表（财务） | `cw_fyhzb` |
| 订单结算主表（销售&车队长&财务） | `cw_ddjszb` |
| 智能表6 | 未建表 |
| 客户资金流水表（财务） | `cw_khzjlsb` |
| 调车成本表（车队长&财务） | `cw_dccbb` |
| 车辆维修成本表 | `cw_wxcbb` |
| 出车交付中心ing_1 | 未建表 |
| 押金管理表 | `cw_yjglb` |
| 挂靠合作明细表(副本) | `cw_gkhzmxb` |
| 挂靠合作明细表 | `cw_gkhzmxb` |
| 智能表4 | 未建表 |
| 表单1 | 未建表 |
| 维修报备申请表 | `cw_wxbbsqb` |
| 长租车管理表 | 未建表 |
| 在外车辆（包月） | `cw_zwclb` |
| 回车违章情况(车队长) | `cw_hcwzqkb` |
| 挂靠车主（车队长） | `cw_gkczb` |
| 车辆异常ing（车队长） | `cw_clycb` |
| 客户主档案（销售） | `cw_khzdab` |
| 车辆折旧台账 | 未建表 |
| 酒店付款台账 | `cw_jdfktzb` |
| 挂靠车辆成本利润表 | `cw_gkclcblrb` |
| 自有车辆成本利润表 | `cw_zyclcblrb` |
| 板车费用明细表（车队长） | `cw_bcfymxb` |
| GPS付款 | `cw_gpsfkb` |
| 代驾费用明细表（财务） | `cw_djfymxb` |
| 报销费用明细（财务） | `cw_bxfymxb` |
| 智能表5 | 未建表 |
| 维修成本表(车队长&财务) | `cw_wxcbb` |
| 配件采购表（车队长&财务） | `cw_pjcgb` |
| 保险&续费记录（车队长） | `cw_bxxfjlb` |
| 税务凭证（财务） | `cw_swpzb` |
| 运营杂费报销表（财务） | `cw_yyzfbxb` |
| 工资报表 | `cw_gzbb` |
...
- 图片字段只能上传图片 MIME 类型；文件字段不能上传图片 MIME 类型。
- 图片字段上传时，前端先转换为 WebP，最长边压缩到 1600px 以内，下载时也下载 WebP 文件。
- 图片弹窗内点击图片可使用历史图片预览同款样式预览，标题显示图片文件名，不显示推送按钮。
- 文件弹窗内点击文件可打开文件预览弹窗，标题显示文件名，底部提供下载按钮；Excel/CSV 渲染为表格预览，PDF、文本等浏览器可内嵌类型直接预览，Word、压缩包等不强制预览，显示下载提示。
- 用户登录会写入 `user_login_log`；中台增删改会写入 `middle_operation_log`。
...
- `user_login_log`：记录用户登录时间、用户 ID、账号、显示名和权限。
- `middle_operation_log`：记录中台操作时间、用户 ID、账号、显示名、操作表、操作类型、主键字段、主键值和变更内容。
...
修改：
- 中台数据读取支持关联 `cw_formula_view_原表名` 公式视图。
- 前端显示中台字段时，后端优先返回原业务表字段值；原字段为空时，再使用公式视图中的同名字段计算值。
- 公式视图只作为显示兜底来源，主表仍以原业务表为准，避免视图缺少普通字段时影响中台列表。
- 修复公式视图中日期/时间字段为空字符串时，中台加载报 `Incorrect DATETIME value: ''` 的问题。
- 企业微信公式转换时，字段等于空字符串会转换为 `empty(字段)`，避免日期/时间字段生成 `eq(字段,"")`。
