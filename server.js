...
async function saveZdRecord(payload) {
  ...
  try {
    ...
    const values = [
      ...
      payload.checkDamage ?? payload.checkScratch,
      '',
      payload.checkViolation ?? payload.checkAccident,
      '',
      '',
...
async function ensureStatementTable(conn) {
  await conn.execute(`CREATE TABLE IF NOT EXISTS tp_dzd (
    ...
    check_scratch VARCHAR(10) NOT NULL DEFAULT '' COMMENT '车损',
    check_mark VARCHAR(10) NOT NULL DEFAULT '' COMMENT '预留检查项',
    check_accident VARCHAR(20) NOT NULL DEFAULT '' COMMENT '额外费用',
    check_over_mileage VARCHAR(10) NOT NULL DEFAULT '' COMMENT '预留检查项',
    check_overtime VARCHAR(10) NOT NULL DEFAULT '' COMMENT '预留检查项',
